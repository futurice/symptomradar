    ...
    s3PutJsonHelper(app.s3Client, {
      ...
      Body: {
        meta: {
          ...
          generated: new Date().toISOString(),
          link: `https://${app.constants.domainName}/city_level_general_results.json`,
        },
        data: cityLevelData,
      },
    }),
...
export async function updateOpenDataIndex(app: App) {
  //
  // Fetch data files
  ...
  for (const filename of openDataFileNames) {
    const data = await s3GetJsonHelper(app.s3Client, { Bucket: app.s3Buckets.openData, Key: `${filename}.json` });
  ...
  await s3PutJsonHelper(app.s3Client, { Bucket: app.s3Buckets.openData, Key: 'index.json', Body: openData });
}

async function mapPostalCodeLevelToCityLevelData(app: App, postalCodeLevelData: any[]) {
  const postalCodeCityMappings = await s3GetJsonHelper(app.s3Client, {
    Bucket: app.s3Buckets.openData,
    Key: 'postalcode_city_mappings.json',
  });

  const populationPerCity = await s3GetJsonHelper(app.s3Client, {
    Bucket: app.s3Buckets.openData,
    Key: 'population_per_city.json',
  });
...
async function s3GetJsonHelper(s3: AWS.S3, params: AWS.S3.GetObjectRequest) {
...
async function s3PutJsonHelper(s3: AWS.S3, params: AWS.S3.PutObjectRequest) {
  return await s3
    .putObject({
      ...params,
