...
import { App, s3PutJsonHelper, s3GetJsonHelper } from './app';
...
export async function updateOpenDataIndex(app: App) {
  ...
  for (const filename of openDataFileNames) {
    const data = await s3GetJsonHelper(app.s3Client, { Bucket: app.constants.openDataBucket, Key: `${filename}.json` });
